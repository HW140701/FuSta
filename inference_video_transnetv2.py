...
sys.path.append('./CVPR2020CODE_yulunliu_modified')
...
def calc_flow(flow_model, img1, img2):
    with torch.no_grad():
...
class StablizeVideoWithTransNetV2():
    def __init__(self, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path, transnetv2_onnx_path, fusta_args, fusta_flow_model_weight_path):
        if not os.path.exists(build_encoder_weight_path):
            raise FileExistsError(f'{build_encoder_weight_path} is not exist')

        if not os.path.exists(build_decoder_weight_path):
            raise FileExistsError(f'{build_decoder_weight_path} is not exist')

        if not os.path.exists(flow_model_weight_path):
            raise FileExistsError(f'{flow_model_weight_path} is not exist')

        if not os.path.exists(pc_u_path):
            raise FileExistsError(f'{pc_u_path} is not exist')

        if not os.path.exists(pc_v_path):
            raise FileExistsError(f'{pc_v_path} is not exist')

        if not os.path.exists(mpi_net35_weight_path):
            raise FileExistsError(f'{mpi_net35_weight_path} is not exist')

        if not os.path.exists(transnetv2_onnx_path):
            raise FileExistsError(f'{transnetv2_onnx_path} is not exist')

        # if not os.path.exists(fusta_arbitrary_model_weight_path):
        #     raise FileExistsError(f'{fusta_arbitrary_model_weight_path} is not exist')

        if not os.path.exists(fusta_flow_model_weight_path):
            raise FileExistsError(f'{fusta_flow_model_weight_path} is not exist')

        # 构建求解仿射场类
        self.build_encoder_weight_path = build_encoder_weight_path
        self.build_decoder_weight_path = build_decoder_weight_path
        self.flow_model_weight_path = flow_model_weight_path
        self.pc_u_path = pc_u_path
        self.pc_v_path = pc_v_path
        self.mpi_net35_weight_path = mpi_net35_weight_path

        self.calculate_warping_field = CalculateVideoWrapingField(build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path)
        if self.calculate_warping_field is None:
            raise Exception('init CalculateVideoWrapingField failed')

        # 构建求解transnetv2类
        self.transnetv2 = TransNetV2_Onnx(transnetv2_onnx_path)
        if self.transnetv2 is None:
            raise Exception('init TransNetV2_Onnx failed')

        # 加载fusta相关模型
        self.fusta_args = fusta_args
        self.fusta_arbitrary_model = models_arbitrary.Model(fusta_args)
        checkpoint = torch.load(args.load)
        self.fusta_arbitrary_model.load(checkpoint['state_dict'])
        self.fusta_arbitrary_model.eval()

        self.fusta_flow_model = torch.nn.DataParallel(RAFT(fusta_args))
        self.fusta_flow_model.load_state_dict(torch.load(fusta_flow_model_weight_path))
        self.fusta_flow_model = self.fusta_flow_model.module
    ...
    def fusta_stablize_video(self, input_video_images_folder, output_video_warping_field, output_video_stable_images):
        ...
        assert (GAUSSIAN_FILTER_KSIZE - 1) // 2 % self.fusta_args.temporal_step == 0

        with torch.no_grad():
            all_imgs = sorted(glob.glob(os.path.join(input_video_images_folder, '*.png')))  # all pngs in a sequence
            ...
            all_imgs = [first_frame] * (GAUSSIAN_FILTER_KSIZE // 2) + all_imgs + [last_frame] * (
                        GAUSSIAN_FILTER_KSIZE // 2)
            ...
            for idx in range(GAUSSIAN_FILTER_KSIZE // 2, (GAUSSIAN_FILTER_KSIZE // 2) + original_length):
                ...
                if int(img_name[:-4]) == 0:
                    ...
                    if os.path.isfile(os.path.join(output_video_warping_field,
                                                   str(int(img_name[:-4]) - 1).zfill(
                                                       5) + '_H_inv.npy')) and os.path.isfile(
                        os.path.join(output_video_warping_field, str(int(img_name[:-4]) - 1).zfill(5) + '.npy')):
                        tenH_inv = torch.FloatTensor(np.ascontiguousarray(read_homography(
                            os.path.join(output_video_warping_field,
                ...
                for frame_shift in range(-(GAUSSIAN_FILTER_KSIZE // 2), (GAUSSIAN_FILTER_KSIZE // 2) + 1, int(args.temporal_step)):
                    ...
                    ref_frame = all_imgs[idx + frame_shift]
                    ref_frame_name = os.path.split(ref_frame)[-1]
                    forward_flow = calc_flow(self.fusta_flow_model, ref_frame, keyframe)
                    ...
                    backward_flow = calc_flow(self.fusta_flow_model, keyframe, ref_frame)
                ...
                input_frames = [
                    x[:, :, boundary_cropping_h:-boundary_cropping_h, boundary_cropping_w:-boundary_cropping_w]
                    for x in input_frames]
                F_kprime_to_k = F_kprime_to_k[:, :, boundary_cropping_h:-boundary_cropping_h,
                                boundary_cropping_w:-boundary_cropping_w]
                forward_flows = [
                    x[:, :, boundary_cropping_h:-boundary_cropping_h, boundary_cropping_w:-boundary_cropping_w]
                    for x in forward_flows]
                backward_flows = [
                    x[:, :, boundary_cropping_h:-boundary_cropping_h, boundary_cropping_w:-boundary_cropping_w] for x in
                    backward_flows]

                frame_out = self.fusta_arbitrary_model(input_frames, F_kprime_to_k, forward_flows, backward_flows)
                ...
                for iiii in range(len(large_mask_chain)):
                    ...
                    for uu in range(-maximum_movement, maximum_movement + 1):
                        ...
                        for vv in range(-maximum_movement, maximum_movement + 1):
                            ...
                            cropped_mask = large_mask_chain[iiii][:, :, HHH + motion_vector_u:-HHH + motion_vector_u,
                            ...
                            data_term[iiii, (uu + maximum_movement) * (
                    ...
                    for vv in range(-maximum_movement, maximum_movement + 1):
                        ...
                        for uuuu in range(-maximum_movement, maximum_movement + 1):
                            for vvvv in range(-maximum_movement, maximum_movement + 1):
                                smoothness_term[
                                    (uu + maximum_movement) * (2 * maximum_movement + 1) + vv + maximum_movement, (
                                            uuuu + maximum_movement) * (
                                            2 * maximum_movement + 1) + vvvv + maximum_movement] = (
            ...
            for iiii in range(len(large_mask_chain)):
                ...
                # if OOM
                imwrite(
                    torch.from_numpy(np.load(output_frames[iiii]))[:, :, HHH + motion_vector_u:-HHH + motion_vector_u,
                    WWW + motion_vector_v:-WWW + motion_vector_v],
                    os.path.join(output_video_stable_images, str(iiii + 1).zfill(5) + '.png'), range=(0, 1))
...
def synthetic_stabilized_video(input_original_video_path, output_video_folder, output_video_stable_imags):
    if not os.path.exists(input_original_video_path):
        raise ValueError(f'Input video path: {input_original_video_path} does not exist')

    if not os.path.exists(output_video_folder):
        raise ValueError(f'Output video folder: {output_video_folder} does not exist')

    output_stable_video_folder = os.path.join(output_video_folder, 'stable_video')
    if not os.path.exists(output_stable_video_folder):
        os.makedirs(output_stable_video_folder)

    input_video_name = os.path.basename(input_original_video_path).split('.')[0]
    output_stable_video_path = os.path.join(output_stable_video_folder, input_video_name + '_stable.mp4')

    video_read_cap = cv2.VideoCapture(input_original_video_path)
    fps = video_read_cap.get(cv2.CAP_PROP_FPS)
    frame_width = int(video_read_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    frame_height = int(video_read_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    frame_count = int(video_read_cap.get(cv2.CAP_PROP_FRAME_COUNT))

    # 获取所有稳定后的图片
    all_stable_images = sorted(glob.glob(os.path.join(output_video_stable_imags, '*.png')))

    #if len(all_stable_images) != frame_count:
        #raise ValueError(f'Frame count of original video: {frame_count} does not match the number of stable images: {len(all_stable_images)}')

    video_write_cap = cv2.VideoWriter(output_stable_video_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height))

    for i, stable_image in enumerate(all_stable_images):
        print(f'Writing frame {i+1}/{frame_count}')
        frame = cv2.imread(stable_image)
        video_write_cap.write(frame)

    video_read_cap.release()
    video_write_cap.release()

    print(f'stable video saved at {output_stable_video_path}')

    return output_stable_video_path

def combine_video(input_video_path1, input_video_path2, output_video_folder):
    if not os.path.exists(input_video_path1):
        raise ValueError(f'Input video path: {input_video_path1} does not exist')

    if not os.path.exists(input_video_path2):
        raise ValueError(f'Input video path: {input_video_path2} does not exist')

    if not os.path.exists(output_video_folder):
        raise ValueError(f'Output video folder: {output_video_folder} does not exist')

    output_video_combine_folder = os.path.join(output_video_folder, 'combined_video')
    if not os.path.exists(output_video_combine_folder):
        os.makedirs(output_video_combine_folder)

    output_video_path = os.path.join(output_video_combine_folder, 'combined_video.mp4')

    # 加载两个视频文件
    video1 = VideoFileClip(input_video_path1)
    video2 = VideoFileClip(input_video_path2)

    # 调整视频大小，使它们的高度相同
    max_height = max(video1.h, video2.h)
    video1_resized = video1.resize(height=max_height)
    video2_resized = video2.resize(height=max_height)

    # 创建一个左右拼接的视频数组
    final_clip = clips_array([[video1_resized, video2_resized]])

    # 保留声音
    final_clip = final_clip.set_audio(video1_resized.audio)
...
if __name__ == '__main__':
    ...
    build_decoder_weight_path = './CVPR2020CODE_yulunliu_modified/baseline-resnet50dilated-ppm_deepsup/decoder_epoch_20.pth'
    flow_model_weight_path = './CVPR2020CODE_yulunliu_modified/FlowNet2_checkpoint.pth.tar'
    pc_u_path = './CVPR2020CODE_yulunliu_modified/PC_U.npy'
    pc_v_path = './CVPR2020CODE_yulunliu_modified/PC_V.npy'
    mpi_net35_weight_path = './CVPR2020CODE_yulunliu_modified/checkpt.pt'
    transnetv2_onnx_path = './transnetv2/transnetv2.onnx'
    fusta_flow_model_weight_path = './raft_models/raft-things.pth'

    stablize_video_with_transnetv2 = StablizeVideoWithTransNetV2(build_encoder_weight_path=build_encoder_weight_path,
                                                                 build_decoder_weight_path=build_decoder_weight_path,
                                                                 flow_model_weight_path=flow_model_weight_path,
                                                                 pc_u_path=pc_u_path,
                                                                 pc_v_path=pc_v_path,
                                                                 mpi_net35_weight_path=mpi_net35_weight_path,
                                                                 transnetv2_onnx_path=transnetv2_onnx_path,
                                                                 fusta_args=args,
