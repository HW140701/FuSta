...
def empty_folder_completely(folder_path):
    try:
        # 删除整个文件夹及其内容
        ...
        print(f'Failed to empty {folder_path}. Reason: {e}')
...
def compute_mask(segmentation_module, img):
...
counter_person = 1
def compute_flow_seg(flow_model, base, segmentation_module, video, H, start):
    ...
    for i in range(video.shape[0]):
        ...
        ss, ss_person = compute_mask(segmentation_module, video[i, :, :, :])
...
def compute_H(input_path):
    ...
    all_frames = sorted(glob.glob(os.path.join(input_path, '*.png')))
...
# def convert_video_to_images(input_video_path, output_folder):
#     # 检查输出文件夹是否存在，如果存在则清空其中的图片文件
#     if os.path.exists(output_folder):
#         # for filename in os.listdir(output_folder):
#         #     file_path = os.path.join(output_folder, filename)
#         #     if os.path.isfile(file_path) and filename.lower().endswith(('.png', '.jpg', '.jpeg')):
#         #         os.remove(file_path)
#         empty_folder_completely(output_folder)
#     else:
#         # 如果文件夹不存在则创建
...
#
#         # 生成图像文件名
#         image_filename = os.path.join(output_folder, f"{frame_count:05d}.png")
#
#         # 保存当前帧为图像
...
#     cap.release()
#
#     print(f"Converted {frame_count} frames to images in {output_folder}")
...
def convert_video_to_images(input_video_path, output_folder):
    ...
    try:
        ...
        # 获取转换后的帧数
        frame_count = len([name for name in os.listdir(output_folder) if name.lower().endswith('.png')])

        print(f"Converted {frame_count} frames to images in {output_folder}")
    except subprocess.CalledProcessError as e:
        print(f"Error converting video to images: {e}")
    except FileNotFoundError:
        print("ffmpeg not found. Please make sure ffmpeg is installed and added to your system's PATH.")

def calculate_video_warping_field(input_video, output_folder, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path):
    if not os.path.exists(input_video):
        raise Exception(f'file {input_video} is not exist')
    ...
    # 构建该视频对应的输出文件夹
    output_video_floder = os.path.join(output_folder, input_video_name)
    if not os.path.exists(output_video_floder):
        os.makedirs(output_video_floder)

    # 构建该视频对应的warping field文件夹
    output_video_warping_field_floder = os.path.join(output_video_floder, 'warping_field')
    if not os.path.exists(output_video_warping_field_floder):
        os.makedirs(output_video_warping_field_floder)
    else:
        empty_folder_completely(output_video_warping_field_floder)

    # 构建该视频对应的result video文件夹
    output_video_result_video_folder = os.path.join(output_video_floder, 'result_video')
    if not os.path.exists(output_video_result_video_folder):
        os.makedirs(output_video_result_video_folder)

    # 分割视频到图片
    input_video_split_image_save_folder = os.path.join(output_video_floder, 'input_video_split_image')
    if not os.path.exists(input_video_split_image_save_folder):
        os.makedirs(input_video_split_image_save_folder)

    print(f'convert video {input_video} to images floder {input_video_split_image_save_folder}')
    convert_video_to_images(input_video, input_video_split_image_save_folder)
    ...
    net_encoder = builder.build_encoder(
        ...
        fc_dim=2048,
        weights=build_encoder_weight_path)
    net_decoder = builder.build_decoder(
        arch='ppm_deepsup',
        fc_dim=2048,
        num_class=150,
        weights=build_decoder_weight_path,
        use_softmax=True)
    ...
    with torch.no_grad():
        ...
        checkpoint = torch.load(flow_model_weight_path)  #
        ...
        if os.path.isfile(mpi_net35_weight_path):
            model.load_state_dict(torch.load(mpi_net35_weight_path))
            print('loaded!!!!!!!!!!!!!!!!!')
        model.eval()


        H, H_inv = compute_H(input_video_split_image_save_folder)

        if not os.path.exists(output_video_warping_field_floder):
            os.makedirs(output_video_warping_field_floder)
        for iii in range(H_inv.shape[0]):
            #np.save(output_video_warping_field_floder + str(iii).zfill(5) + '_H_inv.npy', H_inv[iii, :, :])
            np.save(os.path.join(output_video_warping_field_floder, str(iii).zfill(5) + '_H_inv.npy'), H_inv[iii, :, :])

        # vid_reader = skvideo.io.FFmpegReader(video_path)
        # (numframe, height, width, _) = vid_reader.getShape()
        all_images = sorted(glob.glob(os.path.join(input_video_split_image_save_folder, '*.png')))
        numframe = len(all_images)
        tmp_img = cv2.imread(all_images[0])
        height = tmp_img.shape[0]
        width = tmp_img.shape[1]
        ...
        writer = cv2.VideoWriter(os.path.join(output_video_result_video_folder, input_video_name+'_out.avi'), cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 29, (832 + 2 * margin, 448 + 2 * margin))  # vid_reader.get(cv2.CAP_PROP_FPS)
        writer_mask = cv2.VideoWriter(os.path.join(output_video_result_video_folder, input_video_name+'_out_mask.avi'), cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 29, (832 + 2 * margin, 448 + 2 * margin))  # vid_reader.get(cv2.CAP_PROP_FPS)
        counter = 1
        counter2 = 1
        if not os.path.exists(output_video_warping_field_floder):
            os.makedirs(output_video_warping_field_floder)
        #np.save(output_video_warping_field_floder + str(0).zfill(5) + '.npy', torch.zeros(2, height, width).permute(1, 2, 0).cpu().numpy())
        np.save(os.path.join(output_video_warping_field_floder, str(0).zfill(5) + '.npy'),torch.zeros(2, height, width).permute(1, 2, 0).cpu().numpy())

        for ite2 in range(int(nseg)):
            all_frames = sorted(glob.glob(os.path.join(input_video_split_image_save_folder, '*.png')))
            ...
            optic, video, mask, mask_object = compute_flow_seg(flow_model, base, segmentation_module, video, H, ite2 * (SEG - 1))
            ...
            for i in range(result.shape[0]):
                ...
                np.save(os.path.join(output_video_warping_field_floder, str(counter).zfill(5) + '.npy'), warping_fields[i, :, :, :].permute(1, 2, 0).cpu().numpy())
...
class CalculateVideoWrapingField():
    def __init__(self, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path):
        ...
        crit = torch.nn.NLLLoss(ignore_index=-1)
        self.segmentation_module = SegmentationModule(net_encoder, net_decoder, crit).to(self.device)
        self.segmentation_module.eval()

        # 加载FlowNet2模型
        self.flow_model = models_flownet.FlowNet2().to(self.device)
        flow_model_checkpoint = torch.load(flow_model_weight_path)
        self.flow_model.load_state_dict(flow_model_checkpoint['state_dict'])
        self.flow_model.eval()

        # 加载U和V
        self.U = np.load(pc_u_path)
        self.V = np.load(pc_v_path)

        # 加载mpi_net35
        self.mpi_net35_model = torch.nn.DataParallel(mpi_net35.mpinet(in_channels=60, out_channels=38).to(self.device))
        if os.path.isfile(mpi_net35_weight_path):
            self.mpi_net35_model.load_state_dict(torch.load(mpi_net35_weight_path))
            print('loaded!!!!!!!!!!!!!!!!!')
        self.mpi_net35_model.eval()

    def inference_video(self, input_video, output_folder):
        if not os.path.exists(input_video):
            raise FileExistsError(f'file {input_video} is not exist')
            ...
            for ite2 in range(int(nseg)):
                ...
                for i in range(result.shape[0]):
                    ...
                    if not os.path.exists(output_video_warping_field_floder):
                        os.makedirs(output_video_warping_field_floder)
                    # np.save(output_video_warping_field_floder + str(counter).zfill(5) + '.npy', warping_fields[i, :, :, :].permute(1, 2, 0).cpu().numpy())
                    np.save(os.path.join(output_video_warping_field_floder, str(counter).zfill(5) + '.npy'),
...
if __name__ == '__main__':
    ...
    build_encoder_weight_path = 'baseline-resnet50dilated-ppm_deepsup/encoder_epoch_20.pth'
    build_decoder_weight_path = 'baseline-resnet50dilated-ppm_deepsup/decoder_epoch_20.pth'
    flow_model_weight_path = 'FlowNet2_checkpoint.pth.tar'
    pc_u_path = 'PC_U.npy'
    pc_v_path = 'PC_V.npy'
    mpi_net35_weight_path = 'checkpt.pt'


    #calculate_video_warping_field(input_video_path, output_folder, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path)

    calculate_video_warping_field = CalculateVideoWrapingField(build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path)
    calculate_video_warping_field.inference_video(input_video_path, output_folder)