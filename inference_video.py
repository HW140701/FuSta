...
def stable_video(args, input_video_path, output_folder, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path):
    if not os.path.exists(input_video_path):
        raise ValueError(f'Input video path: {input_video_path} does not exist')
    ...
    input_video_images_folder, output_video_warping_field = calculate_video_warping_field.inference_video(input_video_path, output_folder)

    #input_video_images_folder, output_video_warping_field = calculate_video_warping_field(input_video_path, output_folder, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path)
    ...
    with torch.no_grad():
        ...
        for idx in range(GAUSSIAN_FILTER_KSIZE // 2, (GAUSSIAN_FILTER_KSIZE // 2) + original_length):
            ...
            for frame_shift in range(-(GAUSSIAN_FILTER_KSIZE // 2), (GAUSSIAN_FILTER_KSIZE // 2) + 1,
                ...
                ref_frame = all_imgs[idx + frame_shift]
                ref_frame_name = os.path.split(ref_frame)[-1]
                forward_flow = calc_flow(flow_model, ref_frame, keyframe)
                ...
                backward_flow = calc_flow(flow_model, keyframe, ref_frame)
            ...
            input_frames = [x[:, :, boundary_cropping_h:-boundary_cropping_h, boundary_cropping_w:-boundary_cropping_w]
            ...
            forward_flows = [x[:, :, boundary_cropping_h:-boundary_cropping_h, boundary_cropping_w:-boundary_cropping_w]
        ...
        for iiii in range(len(large_mask_chain)):
            ...
            # if OOM
            imwrite(torch.from_numpy(np.load(output_frames[iiii]))[:, :, HHH + motion_vector_u:-HHH + motion_vector_u,
                    WWW + motion_vector_v:-WWW + motion_vector_v],
...
if __name__ == '__main__':
    ...
    stable_video(args, input_video_path, output_folder, build_encoder_weight_path, build_decoder_weight_path, flow_model_weight_path, pc_u_path, pc_v_path, mpi_net35_weight_path)
